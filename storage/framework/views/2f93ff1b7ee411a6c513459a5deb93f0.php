...
<?php $__env->startSection('title', 'Review Dokumen'); ?>
...
<?php $__env->startSection('content'); ?>
    <div class="review-back-header">
        <div>
            <a href="<?php echo e(route('employee.tasks.show', $publicationTeam->id)); ?>" class="back-link">
                <i class="bi bi-arrow-left"></i> Kembali ke Detail Tugas
            </a>
            <div class="pub-title mt-1"><?php echo e($publication->nama_publikasi); ?></div>
    ...
    <div class="review-container">
        ...
        <div class="pdf-panel">
            <?php if($isGDriveLink): ?>
                <div class="pdf-toolbar">
                    ...
                    <div>
                        <a href="<?php echo e($pdfDocument->external_url); ?>" target="_blank" class="btn btn-sm" style="background: rgba(255,255,255,0.1); color: #fff; border: 1px solid rgba(255,255,255,0.2); font-size: 12px;">
                            <i class="bi bi-box-arrow-up-right me-1"></i> Buka Google Drive
                        </a>
                    </div>
                </div>
                <div class="pdf-viewport p-0" style="background: #e2e8f0;">
                    <iframe src="<?php echo e($gDriveUrl); ?>" style="width: 100%; height: 100%; border: none;" allow="autoplay"></iframe>
                </div>
            <?php elseif($pdfDocument && !$pdfDocument->is_link): ?>
                <div class="pdf-toolbar">
                    ...
                    <div class="file-name" title="<?php echo e($pdfDocument->file_original_name); ?>">
                        <i class="bi bi-file-earmark-pdf me-1"></i><?php echo e($pdfDocument->file_original_name); ?>
            ...
            <?php elseif($pdfDocument && $pdfDocument->is_link): ?>
                <div class="pdf-empty">
                    ...
                    <div>
                        ...
                        <a href="<?php echo e($pdfDocument->external_url); ?>" target="_blank" class="btn btn-outline-primary mt-2">
        ...
        <div class="notes-panel">
            <div class="notes-header">
                ...
                <div class="notes-stats">
                    <span class="stat-badge danger">
                        ...
                        <?php echo e($noteStats['belum_diperbaiki']); ?> Belum
                    </span>
                    <span class="stat-badge warning">
                        <i class="bi bi-circle-fill" style="font-size:6px"></i>
                        <?php echo e($noteStats['sudah_diperbaiki']); ?> Diperbaiki
                    </span>
                    <span class="stat-badge success">
                        <i class="bi bi-circle-fill" style="font-size:6px"></i>
                        <?php echo e($noteStats['diverifikasi']); ?> Diverifikasi
                    </span>
                </div>
            </div>
            ...
            <div class="notes-filter-bar">
                <button type="button" class="filter-btn active" data-filter="all">Semua (<?php echo e($noteStats['total']); ?>)</button>
                <button type="button" class="filter-btn" data-filter="belum_diperbaiki">Belum (<?php echo e($noteStats['belum_diperbaiki']); ?>)</button>
                <button type="button" class="filter-btn" data-filter="sudah_diperbaiki">Diperbaiki (<?php echo e($noteStats['sudah_diperbaiki']); ?>)</button>
                <button type="button" class="filter-btn" data-filter="diverifikasi">Diverifikasi (<?php echo e($noteStats['diverifikasi']); ?>)</button>
            </div>

            
            <div class="notes-list" id="notesList">
                <?php $__empty_1 = true; $__currentLoopData = $reviewNotes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $note): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="note-card" data-page="<?php echo e($note->page_number); ?>" data-status="<?php echo e($note->status); ?>">
                        <div class="d-flex align-items-start justify-content-between gap-2">
                            <div>
                                <span class="note-page-badge">
                                    <i class="bi bi-file-earmark"></i> Hal. <?php echo e($note->page_number); ?>

                                </span>
                            </div>
                            <span class="note-status-badge <?php echo e($note->status); ?>">
                                <?php if($note->status === 'belum_diperbaiki'): ?>
                                    <i class="bi bi-circle-fill" style="font-size:5px"></i>
                                <?php elseif($note->status === 'sudah_diperbaiki'): ?>
                                    <i class="bi bi-check" style="font-size:10px"></i>
                                <?php else: ?>
                                    <i class="bi bi-check-all" style="font-size:10px"></i>
                                <?php endif; ?>
                                <?php echo e($note->status_label); ?>

                            </span>
                        </div>

                        <div class="note-section"><?php echo e($note->section_name); ?></div>
                        <div class="note-content"><?php echo e($note->note); ?></div>

                        <div class="note-meta">
                            <div class="note-meta-left">
                                <span class="note-type-badge <?php echo e($note->note_type); ?>">
                                    <i class="bi <?php echo e($note->note_type_icon); ?>" style="font-size:9px"></i>
                                    <?php echo e($note->note_type_label); ?>

                                </span>
                                <span class="note-reviewer">
                                    <i class="bi bi-person-fill"></i> <?php echo e($note->reviewer->name ?? '-'); ?>

                                </span>
                            </div>
                            <span class="note-time"><?php echo e($note->created_at->diffForHumans()); ?></span>
                        </div>

                        
                        <div class="note-actions" onclick="event.stopPropagation()">
                            <?php if($canMarkFixed && $note->status === 'belum_diperbaiki'): ?>
                                <form action="<?php echo e(route('employee.tasks.document-review.update-note', $note->id)); ?>" method="POST" class="d-inline">
                                    <?php echo csrf_field(); ?>
                            ...
                            <?php if($canVerify && $note->status === 'sudah_diperbaiki'): ?>
                                <form action="<?php echo e(route('employee.tasks.document-review.update-note', $note->id)); ?>" method="POST" class="d-inline">
                                    <?php echo csrf_field(); ?>
                                    <?php echo method_field('PUT'); ?>
                            ...
                            <?php if($canVerify && $note->status === 'diverifikasi'): ?>
                                <form action="<?php echo e(route('employee.tasks.document-review.update-note', $note->id)); ?>" method="POST" class="d-inline">
                                    <?php echo csrf_field(); ?>
                                    <?php echo method_field('PUT'); ?>
                            ...
                            <?php if((int) $note->reviewer_id === (int) auth()->id()): ?>
                                <form action="<?php echo e(route('employee.tasks.document-review.destroy-note', $note->id)); ?>" method="POST" class="d-inline"
    ...
    <div class="note-form-overlay" id="noteFormOverlay">
        <div class="note-form-card">
            ...
            <form action="<?php echo e(route('employee.tasks.document-review.store-note', $publicationTeam->id)); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <div class="form-body">
                    <div class="row g-3">
                        <div class="col-6">
                            ...
                            <input type="number" name="page_number" class="form-control" id="formPageNumber"
                                   min="1" value="<?php echo e(old('page_number', 1)); ?>" required>
                            <?php $__errorArgs = ['page_number'];
$__bag = $errors->getBag($__errorArgs[1] ?? 'default');
if ($__bag->has($__errorArgs[0])) :
if (isset($message)) { $__messageOriginal = $message; }
$message = $__bag->first($__errorArgs[0]); ?>
                                <div class="text-danger mt-1" style="font-size:11px"><?php echo e($message); ?></div>
                            <?php unset($message);
...
unset($__errorArgs, $__bag); ?>
                        ...
                        <div class="col-6">
                            ...
                            <select name="note_type" class="form-select" required>
                                <option value="revisi" <?php echo e(old('note_type') === 'revisi' ? 'selected' : ''); ?>>Revisi</option>
                                <option value="saran" <?php echo e(old('note_type') === 'saran' ? 'selected' : ''); ?>>Saran</option>
                                <option value="catatan" <?php echo e(old('note_type') === 'catatan' ? 'selected' : ''); ?>>Catatan</option>
                            </select>
...
unset($__errorArgs, $__bag); ?>
                        ...
                        <div class="col-12">
                            ...
                            <input type="text" name="section_name" class="form-control"
                                   ...
                                   value="<?php echo e(old('section_name')); ?>" required>
...
document.addEventListener('DOMContentLoaded', function () {
    // ── PDF.js Setup ──
    <?php if($pdfDocument && !$pdfDocument->is_link): ?>
    const pdfUrl = <?php echo json_encode(route('employee.tasks.preview-pdf', $pdfDocument->id), 512) ?>;
...
<?php echo $__env->make('layouts.employee', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\manajemen-publikasi-statistik\resources\views/employee/tasks/document_review.blade.php ENDPATH**/ ?>