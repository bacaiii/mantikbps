...
<head>
    ...
    <title>MANTIK - @yield('title', 'Dashboard Pegawai')</title>
    ...
    <aside class="sidebar" id="mantikSidebar">
        ...
        <div class="nav-wrapper">
            <nav class="nav flex-column">
                ...
                <a href="{{ route('employee.knowledge.index') }}"
                   class="nav-link {{ request()->routeIs('employee.knowledge.*') ? 'active' : '' }}"
                   title="Knowledge">
                    <i class="bi bi-link-45deg"></i>
                    <span class="link-text">Knowledge</span>
                </a>

                <a href="{{ route('employee.ready-release.index') }}"
                   class="nav-link {{ request()->routeIs('employee.ready-release.*') ? 'active' : '' }}"
                ...
                <a href="{{ route('employee.tasks.index') }}"
                   class="nav-link {{ request()->routeIs('employee.tasks.*') && !request('task') ? 'active' : '' }}"
                   title="Semua Tugas">
                    <i class="bi bi-list-task"></i>
                    <span class="link-text">Semua Tugas</span>
                </a>

                <a href="{{ route('employee.tasks.index', ['task' => 'penyusun']) }}"
                   class="nav-link {{ request('task') === 'penyusun' ? 'active' : '' }}"
                   title="Tim Penyusun">
                    <i class="bi bi-upload"></i>
                    <span class="link-text">Tim Penyusun</span>
                </a>

                <a href="{{ route('employee.tasks.index', ['task' => 'konten']) }}"
                   class="nav-link {{ request('task') === 'konten' ? 'active' : '' }}"
                   title="Pemeriksaan Konten">
                    <i class="bi bi-file-earmark-check"></i>
                    <span class="link-text">Pemeriksaan Konten</span>
                </a>

                <a href="{{ route('employee.tasks.index', ['task' => 'layout']) }}"
                   class="nav-link {{ request('task') === 'layout' ? 'active' : '' }}"
                   title="Pemeriksaan Layout">
                    <i class="bi bi-ui-checks-grid"></i>
                    <span class="link-text">Pemeriksaan Layout</span>
                </a>

                <a href="{{ route('employee.tasks.index', ['task' => 'infografis']) }}"
                   class="nav-link {{ request('task') === 'infografis' ? 'active' : '' }}"
                   title="Operator Infografis">
                    <i class="bi bi-images"></i>
                    <span class="link-text">Operator Infografis</span>
                </a>

                <a href="{{ route('employee.tasks.index', ['task' => 'website']) }}"
                   class="nav-link {{ request('task') === 'website' ? 'active' : '' }}"
