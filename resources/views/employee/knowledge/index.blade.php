                ...
                <h2 class="accordion-header" id="heading{{ $collapseId }}">
                    <button class="accordion-button collapsed shadow-sm rounded-4 py-3 px-4" type="button" data-bs-toggle="collapse" data-bs-target="#{{ $collapseId }}" aria-expanded="false" aria-controls="{{ $collapseId }}">
                        ...
                        <span class="flex-grow-1">
                            <span class="d-block fw-bold text-dark">{{ $knowledgeLink->title }}</span>
                            <small class="text-muted">{{ $category }}</small>
                        </span>
                    </button>
                </h2>

                <div id="{{ $collapseId }}" class="accordion-collapse collapse" aria-labelledby="heading{{ $collapseId }}" data-bs-parent="#employeeKnowledgeAccordion">
                    <div class="accordion-body bg-white border rounded-4 mt-2 shadow-sm">
                        <p class="text-muted mb-3">{{ $knowledgeLink->description ?: 'Belum ada deskripsi untuk materi ini.' }}</p>
                        <div class="d-flex align-items-center justify-content-between gap-3 flex-wrap">
                            <small class="text-muted">
                                <i class="bi bi-globe2 me-1"></i>
                                {{ \Illuminate\Support\Str::limit($knowledgeLink->url, 72) }}
                            </small>
                            <a href="{{ $knowledgeLink->url }}" target="_blank" rel="noopener" class="btn btn-primary btn-sm rounded-pill px-3">
